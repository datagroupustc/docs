...
- 管理员：
   - 后台管理员：级别最高
   - 政府：对信息进行统计和查询的角色，能够通过系统后门添加自己的模块
   - 第三方审计机构：
   - 模块管理员：待定

**系统拓展性**
- 应考虑用户数据在其他平台，仅在`数海星云`进行交易的情况
- 数据库的分布式存储，缓存系统的分布式

卖家，店主会对自己的商品进行统计。
系统管理员也会进行统计，看到所有人的信息

- **注意的点**：美工，跨平台，系统鲁棒性，高并发，高可用性，第三方存储平台协作
- 付款方式-3331：工程量的三分之一为准，最后一成在实际使用之后在给
- 安全需求：写在合同中，如果发现多少漏洞，所需要给出折扣，根据风险类型和数量决定少多少钱

- 美工人员：天津大学毕业，效果未知
- UI设计区间：三天出稿，调整周期大概为3天
- 负载均衡：zuul
- 合同：排他性，软件著作权，付款方式，分期节点
- 定点调试，现场测试
- 调度算法：xxl job
- 系统缓存：radis作为读缓存，MongoDB作为写缓存
- 分布式：spring cloud
- 冗余设计
- 安全模块：spring security；
- 前端：react
- 前端的风格要改变
- **文件安全**：我们要提供文件的上传规范，安全标准，然后他们要调用第三方的接口作为中间层。
...
- **附加需求**：
  - 由卖家指定待拍卖的数据集，份数，起止时间，起拍价，最小加价，拍卖模式。由卖家指定买家能否多次出价，是否需要交竞拍费。
...
- **功能**：接收api调用请求，并进行路由处理、身份验证、参数检查、模型调用、数据库操作等
- **附加需求**：
  - 记录响应时间
  - 通信过程中对用户信息的保密性
  - 请求调度
